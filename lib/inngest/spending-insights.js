    ...
    async ({ step }) => {
        ...
        for (const user of users) {
            ...
            const prompt = `
                ...
                Focus on spending patterns, category breakdowns, and actionable advice for better financial management. Use Indian Rupees (₹) as the currency format, and ensure all amounts are shown in rupees.
                Use a friendly, encouraging tone. Format your response as clean HTML for embedding in an email body. **Do not include any markdown, code blocks, or triple backticks. Only return raw HTML.**
                User spending data:
                ${expenseData}
            ...
            try {
                ...
                let rawOutput = aiResponse.response.candidates[0]?.content.parts[0]?.text ?? "";

                // Strip out code fences like ```html and ```
                const htmlBody = rawOutput
                .replace(/^```html\s*/i, "") // remove ```html at the start
                .replace(/^```/, "")         // remove plain ``` if no 'html'
                .replace(/```$/, "");        // remove closing ``` at the end
